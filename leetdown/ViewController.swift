                                    ...
                                    DispatchQueue.global(qos: .background).async {
                                        ...
                                        if file.fileExists(atPath: "/Applications/leetdown.app/Contents/rsr/libusb_missing") {
                                            DispatchQueue.main.async {
                                                self.spinner.stopAnimation(.none)
                                                self.spinner.isHidden = true
                                                self.ph.stringValue = "Failed to install libusb. Please use brew to install it manually." } } else {
                                                    DispatchQueue.main.async {
                                                        self.spinner.stopAnimation(.none)
                                                        self.spinner.isHidden = true
                                            ...
                                            if modalResponse == .alertFirstButtonReturn {
                                                ...
                                                DispatchQueue.global(qos: .background).async {
                                                    ...
                                                    DispatchQueue.global(qos: .background).async {
                                                        ...
                                                        Process.launchedProcess(launchPath: "/bin/bash", arguments: ["/Applications/leetdown.app/Contents/rsr/ipwndfu.sh"]).waitUntilExit()
                                                    }
                                                                                ...
                                                                                return } } else {
                                                                                    ...
                                                                                    if fileManager.fileExists(atPath: "/Applications/leetdown.app/Contents/rsr/restore_failed") {
                                                                                        DispatchQueue.main.async {
                                                                                            self.spinner.stopAnimation(.none)
                                                                                            self.spinner.isHidden = true
                                                                                            self.ph.stringValue = "Restore failed. Please re-select iPSW to try again."
                                                                                            self.ipswselectiontext.isEnabled = false
                                                                                            self.downgradebutton.isEnabled = false
                                                                                        } } else {
                                                                                            ...
                                                                                            DispatchQueue.main.async {
                                                                                                self.spinner.stopAnimation(.none)
                                                                                                self.spinner.isHidden = true
                                                                                                self.ph.stringValue = "Complete! Please report any issues to @rA9_main"
                                                                                                self.ipswselectiontext.isEnabled = true
                                                                                                self.downgradebutton.isEnabled = false }
                                                                    } } } }
                                            return } } } else {
